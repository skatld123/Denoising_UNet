...
with torch.no_grad():
    ...
    for batch, data in enumerate(loader_test, 1):
        # forward pass
        before_noise = data['label']
        label = data['label'].to(device)
        input = data['input'].to(device)
        
        # normalization된 이미지에 노이즈 입히기   
        after_noise = add_noise(before_noise)
        after_noise = after_noise.to(device)
        ...
        # 노이즈 이미지 그리기
        img_input = after_noise[0]
        img_input = img_input.cpu().detach().cpu()
        ...
        # 정답 그리기
        img_label = label[0]
        img_label = img_label.cpu().detach().numpy()
        ...
        plt.savefig(
            '/root/Denoising_UNet/dn_test_result/%03d.png' % (batch))
